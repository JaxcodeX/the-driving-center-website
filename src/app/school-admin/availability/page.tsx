...
function AvailabilityContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      for (const w of availData) byDay[w.day_of_week] = { start: w.start_time.slice(0, 5), end: w.end_time.slice(0, 5), enabled: true }
      setWindows(byDay); setSelectedInstructor(data[0])
    }
    load()
  }, [schoolId])
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const payload = Object.entries(windows).filter(([, v]) => v.enabled).map(([day, v]) => ({ instructor_id: selectedInstructor.id, day_of_week: parseInt(day), start_time: v.start + ':00', end_time: v.end + ':00' }))
    await fetch('/api/instructor-availability', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-school-id': schoolId! }, body: JSON.stringify({ availability: payload }) })
      ...
      <aside style={{ width: '220px', flexShrink: 0, background: GLASS_BG, backdropFilter: GLASS_BLUR, WebkitBackdropFilter: GLASS_BLUR, borderRight: `1px solid ${GLASS_BORDER}`, display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, height: '100vh', overflowY: 'auto', zIndex: 10 }}>
        <div style={{ padding: '28px 20px 20px', borderBottom: `1px solid ${GLASS_BORDER}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div><div style={{ fontSize: '13px', fontWeight: '600', color: '#FFFFFF' }}>Instructor Availability</div><div style={{ fontSize: '11px', color: TEXT_SECONDARY }}>School Admin</div></div>
          </div>
        </div>
